export const OFFERINGS = [
  {
    title: "On-Demand Charter",
    desc: "Book a private jet anywhere in the world, any time. Transparent hourly pricing and wheels up in as little as four hours.",
    image: "/image/av/jet-on-demand.jpg",
    href: "/on-demand-charter",
  },
  {
    title: "Commercial Aircraft Charter",
    desc: "Charter full commercial aircraft for large groups, corporate teams, and sports delegations with dedicated crews and cabins.",
    image: "/image/av/new-plane.jpg",
    href: "/commercial-charter",
  },
  {
    title: "Cargo Charter",
    desc: "Move time-critical and oversized freight worldwide with tailored cargo solutions and end-to-end logistics handling.",
    image: "/image/av/jet-dusk-blue.jpg",
    href: "/cargo-charter",
  },
  {
    title: "Medical Evacuation",
    desc: "Rapid air ambulance and medevac flights equipped for critical care, with medical teams ready around the clock.",
    image: "/image/av/jet-night.webp",
    href: "/medical-evacuation",
  },
  {
    title: "Hajj Charters",
    desc: "Dedicated pilgrimage charters for Hajj and Umrah, delivering comfortable, seamless travel to the holy cities.",
    image: "/image/av/jet-sunset.webp",
    href: "/hajj-charters",
  },
  {
    title: "Empty Legs",
    desc: "Travel on repositioning flights at a fraction of the price. Same private experience, exceptional value, limited availability.",
    image: "/image/av/jet-empty-legs.jpg",
    href: "/empty-legs",
  },
];

export const SERVICE_LIST = [
  "24/7 Global Charter Desk",
  "Vetted Crews & Operators",
  "Door-to-Door Ground Transfers",
  "Bespoke In-Flight Catering",
  "Pet & Family Friendly",
];

export const SERVICES = [
  {
    icon: "fa-solid fa-plane-up",
    title: "Private Charter",
    desc: "On-demand access to thousands of aircraft worldwide, tailored to your route, schedule, and party size.",
    href: "/on-demand-charter",
  },
  {
    icon: "fa-solid fa-plane-departure",
    title: "Commercial Aircraft Charter",
    desc: "Charter full commercial aircraft for large groups, corporate teams, and sports delegations with dedicated crews and cabins.",
    href: "/commercial-charter",
  },
  {
    icon: "fa-solid fa-boxes-stacked",
    title: "Cargo Charter",
    desc: "Move time-critical and oversized freight worldwide with tailored cargo solutions and end-to-end logistics handling.",
    href: "/cargo-charter",
  },
  {
    icon: "fa-solid fa-kit-medical",
    title: "Medical Evacuation",
    desc: "Rapid air ambulance and medevac flights equipped for critical care, with medical teams ready around the clock.",
    href: "/medical-evacuation",
  },
  {
    icon: "fa-solid fa-kaaba",
    title: "Hajj Charters",
    desc: "Dedicated pilgrimage charters for Hajj and Umrah, delivering comfortable, seamless travel to the holy cities.",
    href: "/hajj-charters",
  },
  {
    icon: "fa-solid fa-tags",
    title: "Empty Legs",
    desc: "Travel on repositioning flights at a fraction of the price. Same private experience, exceptional value, limited availability.",
    href: "/empty-legs",
  },
  {
    icon: "fa-solid fa-briefcase",
    title: "Aircraft Management",
    desc: "Full-service management that maximises your asset and minimises your workload.",
  },
  {
    icon: "fa-solid fa-bell-concierge",
    title: "VIP Concierge",
    desc: "Ground transport, catering, hotels, and itineraries — orchestrated end to end.",
  },
  {
    icon: "fa-solid fa-shield-halved",
    title: "Safety & Compliance",
    desc: "ARGUS & Wyvern-rated operators only, with rigorous crew and maintenance audits.",
  },
  {
    icon: "fa-solid fa-id-card",
    title: "Luxera Access — The Ultimate Jet Card",
    desc: "Fixed hourly rates, guaranteed availability, and priority access through our exclusive membership program.",
    jetCard: true,
  },
];

export const SERVICE_INTRO = {
  subtitle: "PREMIUM SERVICE",
  title: "Every Detail of Your Flight, Handled by One Team.",
  image1: "/image/av/luxury-jet.jpg",
};

export const SERVICE_PRINCIPLES = {
  title: "The Art of Effortless Travel",
  intro:
    "We approach every journey as a bespoke experience, focusing on the following principles to ensure total seamlessness:",
  items: [
    {
      icon: "fa-solid fa-compass",
      title: "Singular Coordination",
      desc: "We act as your dedicated point of contact, ensuring a fluid, uninterrupted experience from the moment you depart until you reach your final destination.",
    },
    {
      icon: "fa-solid fa-route",
      title: "Logistical Precision",
      desc: "Our team manages the complexities of global charter access and rigorously vetted ground transfers, allowing you to move through your journey without friction.",
    },
    {
      icon: "fa-solid fa-heart",
      title: "Anticipatory Care",
      desc: "We curate every element to your specific requirements, from private, in-flight culinary preferences to the comfort of your guests, family, and pets.",
    },
    {
      icon: "fa-solid fa-user-shield",
      title: "Absolute Discretion",
      desc: "Your privacy and peace of mind are foundational to our approach, supported by a standard of care that is both invisible and unwavering.",
    },
  ],
};
